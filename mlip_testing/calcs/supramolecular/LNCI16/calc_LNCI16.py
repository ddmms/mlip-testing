...
class LNCI16Benchmark(zntrack.Node):
    ...
    def benchmark_lnci16(
        ...
        -------
        tuple[pd.DataFrame, list[Atoms]]
            Results DataFrame and list of complex structures.
        """
        ...
        if supports_charges:
            ...
            logging.warning(
                f"  Calculator {model_name} may not support charge handling"
            )
    ...
    def run(self):
        ...
        # Run benchmark
        results_df, complex_atoms = self.benchmark_lnci16(
        ...
        if not results_df.empty:
            results_df.to_csv(write_dir / "lnci16_results.csv", index=False)
