"""Run calculations for PLA15 protein-ligand benchmark."""
...
class PLA15Benchmark(zntrack.Node):
    """
    Benchmark model for PLA15 protein-ligand interactions.

    Evaluates interaction energies for complete active site-ligand interactions.
    ...
    def run(self):
        ...
        # PLA15 specific paths
        pla15_dir = base_dir / "PLA15_pdbs"
        pla15_ref_file = pla15_dir / "reference_energies.txt"

        if not pla15_dir.exists() or not pla15_ref_file.exists():
            logging.warning("PLA15 data directory or reference file not found")
            return
        ...
        pla15_refs = self.parse_pla15_references(pla15_ref_file)
        ...
        for pdb_file in tqdm(pdb_files, desc="PLA15"):
            ...
            try:
                ...
                logging.warning(f"Error processing PLA15 {identifier}: {e}")
                continue

        # Save results
        if results:
            pla15_df = pd.DataFrame(results)
            pla15_df.to_csv(write_dir / "pla15_results.csv", index=False)
            pla15_mae = float(np.abs(pla15_df["error_kcal"]).mean())

            mae_data = {"PLA15_MAE_kcal": pla15_mae}
            print(f"PLA15 MAE: {pla15_mae}")
